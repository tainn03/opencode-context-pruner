...
export async function extractInsightsFromMessages(
  messages: Array<{ id?: string; role: string; content?: string; toolUse?: unknown[] }> | null | undefined,
  ...
  if (focus) {
    filtered = insights.filter(i => i.category === focus || i.content.toLowerCase().includes(focus.toLowerCase()));
...
function detectCategory(
  content: string
): 'security' | 'architecture' | 'bug' | 'performance' | 'dependency' | 'error' | 'refactor' | 'other' {
  const lower = content.toLowerCase();

  if (lower.includes('security') || lower.includes('vulnerability') || lower.includes('exploit') || lower.includes('injection')) {
    return 'security';
  }
  if (lower.includes('architecture') || lower.includes('pattern') || lower.includes('migrat') || lower.includes('design')) {
    return 'architecture';
  }
...
function extractFilePathsFromExamples(examples: string[]): string[] {
  ...
  for (const example of examples) {
    const matches = example.match(/(?:src|lib|app)\/[a-zA-Z0-9\-_.\/]+(\.ts|\.js|\.py|\.java)/gi);
    if (matches) {
 ...
 */
async function extractLSPErrors(
  context: PluginContext,
  config: DreamConfig
): Promise<Insight[]> {
