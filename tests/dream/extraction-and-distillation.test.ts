...
test('Dream End-to-End: Messages → Distilled Entries', async () => {
  ...
  assert.ok(entries.some(e => e.content.includes('✅')), 'Should have symbols');
  assert.ok(entries.some(e => e.category === 'security'), 'Should have security');
  assert.ok(entries.some(e => e.category === 'architecture'), 'Should have architecture');
});
