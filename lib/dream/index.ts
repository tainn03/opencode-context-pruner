...
export async function runDreamConsolidation(
  ...
  try {
    ...
    const entries = await distillEntries(
      insights,
      sessionId,
      config
    );
    ...
    const writeResult = await applyToAgentsMd(
      entries,
      sessionId,
      context
    );
    ...
    const contextSummary = await injectContextMemory(
      sessionId,
      context,
      config
    );
...
async function extractInsights(
  ...
  return insights
    ...
    .slice(0, config.maxEntriesPerRun);
}
