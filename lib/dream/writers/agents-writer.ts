  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (match) {
        ...
        logger.debug(`[Dream] Skipping duplicate entry: ${entry.content.substring(0, 40)}...`);
      } else {
    ...
    if (existingEntries.length > archiveThreshold) {
      const entriesToArchive = existingEntries.splice(0, existingEntries.length - archiveThreshold);
...
// Ensure archive directory exists
    ...
    logger.error(`[Dream] Failed to write AGENTS.md: ${err instanceof Error ? err.message : String(err)}`);
    throw err;
  }
}
