...
function buildResultBox(result: any): string {
  ...
  lines.push(`│ ${pathLabel.padEnd(15)} ${pathValue.substring(0, width - 20).padEnd(width - 20)} │`);
  ...
  if (result.writeResult.archivePath) {
    ...
    lines.push(`│ ${archLabel.padEnd(15)} ${archValue.substring(0, width - 20).padEnd(width - 20)} │`);
  }
