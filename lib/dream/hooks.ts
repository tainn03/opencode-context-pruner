 ...
 */
export async function registerDreamHook(
  plugin: Plugin,
  config: DreamConfig
): Promise<void> {
  if (!config.enabled) {
 ...
 */
export async function registerDreamContextHook(
  plugin: Plugin,
  config: DreamConfig
): Promise<void> {
  if (!config.enabled) {
  ...
  plugin.hook('experimental.chat.messages.transform', async (event: any, context: any) => {
    ...
    if (shouldTriggerDreamOnIdle(event, context, config)) {
      try {
        ...
        logger.error(`[Dream] Idle trigger failed: ${err instanceof Error ? err.message : String(err)}`);
      }
    }
  });
 ...
 */
export async function registerDreamCommand(
  plugin: Plugin,
  config: DreamConfig
): Promise<void> {
  if (!config.enabled) {
